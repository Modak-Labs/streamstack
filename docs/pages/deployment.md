...
Docker image: `harness/docker`. Deploy `StreamStack` on `local`, [aws](/deployment/aws), or [fly](/deployment/fly). Each node: raft, metadata, and the storage engine in one JVM, speaking the [Native](/protocol/native) or [Durable Streams](/protocol/durable-streams) API. Pick the protocol with the tabs on each snippet.
