...
public final class NativeServer implements AutoCloseable {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(NativeServer.class);
    ...
    public NativeServer(ServerConfig config) throws Exception {
        this.config = Objects.requireNonNull(config, "config");
        this.node = new StreamStackNode(config);
        NativeHandler handler = new NativeHandler(
            node.service(),
    ...
    public void start() throws Exception {
        ...
        LOGGER.info(
            "streamstack native server started nodeId={} http={}:{} raft={}:{} storage={} wal={} httpThreads={}",
